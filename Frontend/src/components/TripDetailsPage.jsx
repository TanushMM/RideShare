      ...
      if (sessionStorage.getItem("bookedRide") === null) {
        try {
          ...
          sessionStorage.setItem(
            "searchRide",
            JSON.stringify(response.data.searchRide)
          )
          sessionStorage.setItem(
            "bookedRide",
            JSON.stringify(response.data.poster)
          )
      ...
      <Grid container>
        ...
        >
          ...
          <Typography className="Primary">
            <strong>From: </strong>
            {bookedRide?.from?.location}
          </Typography>
          <Typography className="Primary">
            <strong>To: </strong>
            {bookedRide?.to?.location}
          </Typography>
          <Typography className="Primary">
            <strong>Date: </strong> {bookedRide?.date}
          </Typography>
          <Typography className="Primary">
            <strong>Time: </strong>
            {bookedRide?.time}
          ...
          <Typography className="Primary">
            <strong>Driving Style: </strong> {bookedRide?.drivingStyle}
          </Typography>
          <Typography className="Primary">
            <strong>Seats Available: </strong> {bookedRide?.seats}
          </Typography>
          <Typography className="Primary">
            <strong>Contact: </strong> {bookedRide?.email}
          </Typography>
          ...
          <Box mt={2} flexDirection={"column"} display={"flex"}>
            {timeLeft > 30 ? (
              ...
              <Typography color="red">
                Ride cannot be canceled within 30 minutes of start time.
              </Typography>
            )}
          </Box>
        </Grid>
        ...
        <Grid item xs={12} md={7} p={3}>
          ...
          >
            {loading ? (
              ...
              <Box
                sx={{ height: "80%", mt: 2, borderRadius: "8px", boxShadow: 2 }}
              >
                <GoogleMap
                  mapContainerStyle={{
                    width: "100%",
